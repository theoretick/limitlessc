...
var callback=widget.callback;var element=widget.element;var user=widget.options.user;var repo=widget.options.repo;var branch=widget.options.branch;var avatarSize=widget.options.avatarSize||20;var last=widget.options.last==undefined?0:widget.options.last;var limitMessage=widget.options.limitMessageTo==undefined?0:widget.options.limitMessageTo;element.append('<p>Widget initialization, please wait...</p>');gh.commit.forBranch(user,repo,branch,function(data){var commits=data.commits;var totalCommits=(last<commits.length?last:commits.length);element.empty();var list=$('<ul>').appendTo(element);for(var c=0;c<totalCommits;c++){list.append('<li>'+' '+author(commits[c].author.login)+' committed '+message(commits[c].message,commits[c].url)+' '+when(commits[c].committed_date)+'</li>');}
element.append('<p/>');callback(element);function avatar(email,size){var emailHash=hex_md5(email);return'<img class="github-avatar" src="http://www.gravatar.com/avatar/'+emailHash+'?s='+size+'"/>';}
...
return{run:function(){_widgetRun(this);}};})();$.fn.githubInfoWidget=function(options,callback){this.each(function(){var w=new widget($(this),options,callback);w.run();});return this;}})(jQuery);
